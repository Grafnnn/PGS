...
export function ChangeOrderManagementWorkspace(props: Props) {
  ...
  return <section className="change-management" aria-label="Change Order Management">
    <header className="change-management-header"><div><div className="eyebrow">Управление изменениями v2</div><h3>Изменения, допработы и договорная цена</h3><p>От кандидата до исполнения: стоимость, доказательства, согласование и журнал решений. Утверждение не переписывает ВОР или денежный поток автоматически.</p></div><button className="button secondary compact-button" type="button" onClick={() => props.onNavigate("Процессы")}><GitBranch size={16}/>Матрица согласования</button></header>
    ...
    {props.canEdit && <form className="change-order-form" onSubmit={save}><div className="section-title"><Plus size={18}/><h4>{editingId ? "Редактирование изменения" : "Новый черновик"}</h4></div><div className="change-order-form-grid">
      ...
      <label className="field"><span>Область изменения</span><select value={form.scope} onChange={(event) => setForm({ ...form, scope: event.target.value as FormState["scope"] })}>{Object.entries(scopeLabels).map(([value,label]) => <option value={value} key={value}>{label}</option>)}</select></label>
