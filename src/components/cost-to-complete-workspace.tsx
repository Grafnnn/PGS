...
import { AlertTriangle, Landmark, ListChecks, TrendingDown, Users, WalletCards } from "lucide-react";
...
export function CostToCompleteWorkspace(props: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/projects/${props.project.id}/resources`, { cache: "no-store" })
      ...
      .then((body) => {
        if (active && body) setWorkforce({ resources: body.items ?? [], demands: body.demands ?? [], policy: body.policy ?? null });
      })
      .catch(() => undefined);
    return () => {
      active = false;
  ...
  const model = buildCostToCompleteIntelligence({
    ...props,
    workforceResources: workforce.resources,
    laborDemands: workforce.demands,
    payrollPolicy: workforce.policy
  });
  ...
  return <section className="quality-issues-workspace cost-to-complete-workspace" aria-label="Cost to Complete and Margin Forecast">
    ...
    <div className="quality-issues-grid metrics"><Metric title="Forecast cost" value={money(summary.forecastCost)} detail={`план ${money(summary.plannedCost)}`} tone={summary.tone} /><Metric title="До завершения" value={money(summary.costToComplete)} detail={`${summary.remainingWorkPercent.toFixed(0)}% работ осталось`} tone={summary.tone} /><Metric title="Forecast margin" value={`${summary.forecastMarginPercent.toFixed(1)}%`} detail={money(summary.forecastMargin)} tone={summary.forecastMarginPercent < 5 ? "bad" : summary.tone} /><Metric title="ФОТ работодателя" value={money(summary.payrollEmployerCost)} detail={summary.payrollUncoveredCost ? `вне бюджета ${money(summary.payrollUncoveredCost)}` : `начисления ${money(summary.payrollContributions)}`} tone={summary.payrollUncoveredCost ? "warn" : summary.payrollEmployerCost ? "info" : "neutral"} /><Metric title="Liquidity" value={summary.cashGap < 0 ? money(summary.cashGap) : "без разрыва"} detail={`need ${money(summary.financingNeed)}`} tone={summary.cashGap < 0 ? "bad" : "good"} /></div>
    <div className="quality-issues-grid"><article className="quality-issues-card wide"><div className="section-title"><Landmark size={18} /><h4>Cost forecast by category</h4><button className="button secondary compact-button" type="button" onClick={() => props.onNavigate("ФОТ")}><Users size={15} />ФОТ</button></div><div className="table-wrap"><table><thead><tr><th>Категория</th><th className="numeric">План</th><th className="numeric">Факт</th><th className="numeric">Forecast</th><th className="numeric">Отклонение</th></tr></thead><tbody>{model.categories.length ? model.categories.map((item) => <tr key={item.key}><td><span className={`badge ${badge(item.tone)}`}>{item.label}</span></td><td className="numeric">{money(item.planned)}</td><td className="numeric">{money(item.actual)}</td><td className="numeric">{money(item.forecast)}</td><td className="numeric">{item.deviation > 0 ? "+" : ""}{money(item.deviation)}</td></tr>) : <tr><td colSpan={5}>ВОР пока не загружен: прогноз не рассчитывается.</td></tr>}</tbody></table></div></article><article className="quality-issues-card"><div className="section-title"><AlertTriangle size={18} /><h4>Forecast signals</h4></div><div className="quality-issues-list">{model.signals.length ? model.signals.map((item) => <button className={`quality-issue-item tone-${item.tone}`} key={item.id} type="button" onClick={() => props.onNavigate(item.targetTab)}><strong>{item.title}</strong><small>{item.detail}</small></button>) : <span className="muted">Критичных сигналов в доступном срезе нет.</span>}</div></article><article className="quality-issues-card"><div className="section-title"><WalletCards size={18} /><h4>Margin protection actions</h4></div><div className="quality-issues-action-list">{model.actions.map((item) => <button className={`quality-issues-action priority-${item.priority}`} key={item.title} type="button" onClick={() => props.onNavigate(item.targetTab)}><strong>{item.title}</strong><span>{item.ownerRole} · {item.detail}</span></button>)}</div></article><article className="quality-issues-card wide"><div className="section-title"><AlertTriangle size={18} /><h4>Ограничения v1</h4></div><p className="quality-issues-handoff-copy">{summary.nextStep}</p><ul className="quality-issues-limitations">{model.limitations.map((item) => <li key={item}>{item}</li>)}</ul></article></div>
  </section>;
}
