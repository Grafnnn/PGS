...
import {
  parseDailyReportEquipmentActuals,
  parseDailyReportImpactSummary,
  parseDailyReportMaterialActuals
...
export function serializeProject(project: DbProject): Project {
  return {
    ...
    objectType: project.objectType ?? undefined,
    address: project.address,
    description: project.description ?? undefined,
    contractAmount: num(project.contractAmount),
    vatMode: project.vatMode === "no_vat" ? "no_vat" : "vat",
    vatPercent: project.vatPercent === null ? undefined : num(project.vatPercent),
    startsAt: dateOnly(project.startsAt),
    endsAt: dateOnly(project.endsAt),
    manager: project.manager,
    tenderSource: project.tenderSource ?? undefined,
    paymentNotes: project.paymentNotes ?? undefined,
    volumeChangeMode: project.volumeChangeMode ?? undefined,
    templateId: project.templateId ?? undefined,
    selectedModules: Array.isArray(project.selectedModules) ? project.selectedModules.filter((item): item is string => typeof item === "string") : undefined,
...
export function serializeDailyReport(item: DbDailyReport): DailyReport {
  return {
    ...
    materialActuals: parseDailyReportMaterialActuals(item.materialActuals),
    equipmentActuals: parseDailyReportEquipmentActuals(item.equipmentActuals),
    status: item.status as DailyReport["status"],
    impactStatus: item.impactStatus as DailyReport["impactStatus"],
    impactAppliedAt: item.impactAppliedAt?.toISOString() ?? null,
    impactAppliedBy: item.impactAppliedBy,
    impactSummary: parseDailyReportImpactSummary(item.impactSummary)
  };
}
