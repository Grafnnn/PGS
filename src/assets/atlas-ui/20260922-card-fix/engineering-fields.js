...
(function (root) {
  ...
  function fields(passport, object) {
    ...
    addNumber(dimensions, 'Длина по паспорту (length_mm)', 'length_mm', 'мм');
    addNumber(dimensions, 'Длина по паспорту (length_m)', 'length_m', 'м');
    // Verified own concrete body; the same field on rebar can describe its parent.
    if (p.id === 'R10L_ROOF_MU56_CONCRETE' && p.position === 'Бетон' && /^Бетон(?:\s|$)/u.test(p.material || '') && Array.isArray(p.nominal_body_size_mm) && p.nominal_body_size_mm.length === 3 && p.nominal_body_size_mm.every(v => number(v) !== null)) {
      dimensions.push({label:'Номинальное тело МУ56 по паспорту',value:p.nominal_body_size_mm.map(format).join(' × ')+' мм',field:'nominal_body_size_mm'});
    }
    ...
    const axis = text(object?.albumRecord?.axis);
    if (axis) position.push({label:'Оси по реестру',value:axis,field:'albumRecord.axis'});
